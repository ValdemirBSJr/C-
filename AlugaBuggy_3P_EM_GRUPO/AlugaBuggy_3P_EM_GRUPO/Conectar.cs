            ...
            try
            {
              ...
              //  string encriptado = Crypto.Criptografar(Log.Senha);
                ...
                if (conexao.State == System.Data.ConnectionState.Open) //testa se a conexao esta aberta
                {
                    ...
                    comando.CommandText = "SELECT * FROM cliente WHERE NomeCliente = @LOGIN AND senhaCliente = @SENHA"; //comando para leitura
                    ...
                    while (lercomando.Read())
                    {
                        ...
                        loginGuardado = lercomando["NomeCliente"].ToString();
                        senhaGuardada = lercomando["senhaCliente"].ToString();
                ...
                if ((loginGuardado == Log.Login) && (senhaGuardada == Log.Senha))
                {
                    ...
                    Status.Mensagem = mensage; //aqui chamo a função e seto a string para ela
                ...
                if (conexao.State == System.Data.ConnectionState.Open)
                {
                    ...
                    if ((!String.IsNullOrEmpty(cad.Senha)) && (!String.IsNullOrEmpty(cad.ConfSenha)) && (!String.IsNullOrEmpty(cad.Login))) //se os 3 campos não forem nulos ou vazios e se a senha e confsenha forem iguais
                    {
                        if (cad.Senha == cad.ConfSenha && cad.Login != string.Empty)
                        {
                            ...
                            string inserir = "INSERT INTO cliente (senhaCliente, NomeCliente, FoneCliente, CNHCliente, RGCLiente, CPFCLiente, EnderecoCliente) VALUES (@SENHA, @LOGIN, @FONE, @CNH, @RG, @CPF, @ENDERECO)";

                            MySqlCommand insercaoSQL = new MySqlCommand(inserir, conexao);

                            insercaoSQL.Parameters.AddWithValue("LOGIN", cad.Login);
                            insercaoSQL.Parameters.AddWithValue("SENHA", cad.Senha);
                            insercaoSQL.Parameters.AddWithValue("FONE", cad.Telefone);
                            insercaoSQL.Parameters.AddWithValue("CNH", cad.Cnh);
                            insercaoSQL.Parameters.AddWithValue("RG", cad.Rg);
                            insercaoSQL.Parameters.AddWithValue("CPF", cad.Cpf);
                            insercaoSQL.Parameters.AddWithValue("ENDERECO", cad.Endereco);
        ...
        public void apagaUser(Usuario cad) //metodo para excluir user
        {
            ...
            try
            {
                ...
                if (conexao.State == System.Data.ConnectionState.Open)
                {
                    ...
                    comando.CommandText = "SELECT * FROM cliente WHERE senhaCliente = @SENHA AND NomeCliente = @LOGIN"; //comando para leitura
                    ...
                    comando.Parameters.AddWithValue("SENHA", cad.Senha);
                    ...
                    else
                    {
                            ...
                            string inserir = "DELETE FROM cliente WHERE NomeCliente = @LOGIN";
